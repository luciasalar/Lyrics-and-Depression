...
class GetLabels:
    ...
    def select_quotes(self):
        ...
        file = pd.read_csv(self.path + 'quoteLabel_all_bleu.csv')
        ...
        search_result = all_search.merge(file, on='textID', how='inner')
        search_result.to_csv(self.path + 'all_search/search_result.csv')
    ...
    def merge_counts(self):
        ...
        bleu = pd.read_csv(self.path + 'quoteLabel_all_bleu.csv')
        bleu = bleu[['textID', 'bleu_score']]
        lyrics_fea = counts.merge(bleu, on='textID', how='inner')
    ...
    def annotate_quote_or_lyric(self, input_file, output_file, count, bleu_score):
        """Annotate lyrics or quotes"""
        tag = []
        for c, bleu, quote, in zip(input_file.lyric_count, input_file.bleu_score, input_file.quote_count):
          #  print(bleu)
            if (c > count) & (bleu > bleu_score): # 2
                tag.append('lyrics')
            # elif (c > 0 & int(bleu) > bleu_score):
            #     tag.append('lyrics')
            elif (c < count) & (bleu > bleu_score):
                tag.append('quote')
            else:
                tag.append('NonQuote')
...
def loop_da_grid():
    ...
    # get search results and count keywords, then hash to dict
    search_result = l.select_quotes()
    user_dict = l.hash_results('all_search/search_result.csv')
    ...
    file_exists = os.path.isfile(l.path + '../result/lyrics_accuracy_bleu.csv')
    f = open(l.path + '../result/lyrics_accuracy_bleu.csv', 'a')
    writer_top = csv.writer(f, delimiter=',', quoting=csv.QUOTE_MINIMAL)
    if not file_exists:
        writer_top.writerow(['count'] + ['train_result'] + ['test_result'] + ['time'] )


    for count in experiment['lyrics']['count']:
        train_re = l.annotate_quote_or_lyric(train, 'lyrics_output_train_bleu.csv', count, 0.00000001)
        report_train = l.get_evaluation_train(train_re)
        print(report_train)

        test_re = l.annotate_quote_or_lyric(test, 'lyrics_output_test_bleu.csv', count, 0.00000001)
        report_test = l.get_evaluation_train(test_re)
        
        f = open(l.path + '../result/lyrics_accuracy_bleu.csv', 'a')
        result_row = [[count, pd.DataFrame(report_train), pd.DataFrame(report_test), str(datetime.datetime.now())]]
