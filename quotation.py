...
class Stats:
    ...
    def count_quote_sentiment(self):
        ...
        per_user = positive.groupby(['userid','label']).size().to_frame(name='count_pos').reset_index()
        positive_c = per_user.loc[per_user['label'] == 1]
        # negative
        negative = all_data.loc[all_data['sentiment'] < 0]
        per_user2 = negative.groupby(['userid','label']).size().to_frame(name='counts_neg').reset_index()
        negative_c = per_user2.loc[per_user2['label'] == 1]
        # neutral
        neutral = all_data.loc[all_data['sentiment'] == 0]
        per_user3 = neutral.groupby(['userid','label']).size().to_frame(name='counts_neu').reset_index()
...
class QuoteFeature:
    ...
    def merge_features(self):
        ...
        return quote_fea[['userid','count_pos','counts_neu','counts_neg']]
