...
class MyQuote:
    ...
    def __init__(self, text):
        ...
        self.textID = [] #unique text ID
        self.link = [] # link of the retrieved entry
        self.name = [] # name of the retrieved entries
        self.description = [] # retrieved entry
        self.scores = [] 
        self.bleu = [] #BLEU scores
...
class GetLabels:
    ...
    def get_bleu(self):
        ...
        for item in results:
            ...
            # preprocess documents
            pro_doc1 = self.preprocess2(str1)
            pro_doc2 = self.preprocess2(results[item].quoteText)
            # use spacy model on text
            BLEUscore = nltk.translate.bleu_score.sentence_bleu([pro_doc1], pro_doc2)
            #print(BLEUscore)

            # compute cosine similarity
            results[item].bleu.append(BLEUscore)
           
        return results

    def get_quote_label(self, filename, score):
        ...
        objects = self.get_bleu()
        f = open(self.path + filename, 'w')
        writer = csv.writer(f, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(["textID"] + ["text"] + ["bleu_score"] +
        ['label'])
        # for item in objects:
        #     writer.writerow([objects[item].textID[0]] + [objects[item].quoteText] + [objects[item].scores] + [objects[item].bleu])
       
        for item in objects:
           
           # if cosine similarity > 09=.9
            if objects[item].bleu[0] >= score: #0.988
                writer.writerow([objects[item].textID[0]] + [objects[item].quoteText]  +
                [objects[item].bleu[0]] + ['quote'])
        
            else:
                writer.writerow([objects[item].textID[0]] + [objects[item].quoteText]  +
                [objects[item].bleu[0]] + ['NonQuote'])
        f.close()
        return objects
...
# get cosine similarity score, this table is used as feature directly
    ...
    def get_evaluation_train(self):
        ...
        y_pred_df = pd.read_csv(self.path + 'quoteLabel_train_bleu.csv')
    ...
    def get_evaluation_test(self):
        ...
        y_true_df = y_true_df[['textID', 'human']]
        y_pred_df = pd.read_csv(self.path + 'quoteLabel_test_bleu.csv')
        y_pred_df = y_pred_df[['textID', 'label']]
        result_df = y_true_df.merge(y_pred_df, on='textID', how='inner')
        report = classification_report(result_df['human'], result_df['label'], output_dict=True)
        return report
...
def loop_da_grid():
    ...
    file_exists = os.path.isfile(train.path + '../result/quotation_accuracy_BLEU.csv')
    f = open(train.path + '../result/quotation_accuracy_BLEU.csv', 'a')
    writer_top = csv.writer(f, delimiter=',', quoting=csv.QUOTE_MINIMAL)
    if not file_exists:
        writer_top.writerow(['thredshold'] + ['train_report'] + ['test_report'] +['time'])

    for score in experiment['bleu_score']['thredshold']:

        p_obj = train.get_quote_label('quoteLabel_train_bleu.csv', score)
        p_obj2 = test.get_quote_label('quoteLabel_test_bleu.csv', score)
        ...
        f = open(train.path + '../result/quotation_accuracy_BLEU.csv', 'a')
        result_row = [[score, pd.DataFrame(report_train), pd.DataFrame(report_test), str(datetime.datetime.now())]]
...
g.get_quote_label('quoteLabel_all_bleu.csv', 0.00000001)
...
# train = GetLabels('all_search/test_search.csv')
# p_obj = train.get_quote_label('quoteLabel_test_bleu.csv', 0.0000000001)
